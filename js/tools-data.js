// =========================
// FAHAD TECH - TOOLS DATABASE (FIRESTORE CONNECTED)
// =========================

let toolsData = [];

// Load tools from Firestore
function loadToolsFromFirestore() {
    return new Promise((resolve, reject) => {
        db.collection('tools').get().then((snapshot) => {
            toolsData = [];
            
            if (snapshot.empty) {
                // Firestore empty hai - default tools save karo
                toolsData = getDefaultTools();
                saveDefaultToolsToFirestore();
            } else {
...
// Save default tools to Firestore
function saveDefaultToolsToFirestore() {
    toolsData.forEach((tool) => {
        db.collection('tools').add(tool).then(() => {
            console.log('Tool saved to Firestore:', tool.name);
        }).catch((error) => {
