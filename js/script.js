// =========================
// FAHAD TECH - MAIN SCRIPT (NO AUTH)
// =========================
...
function checkToolAccess(category) {
    ...
    if (currentUserData && currentUserData.accountStatus === 'disabled') { showToast('Account disabled!', 'error'); return; }
    if (!currentUserData || currentUserData.credits < 5) { showInsufficientCredits(5); return; }
    deductCredits(5, 'tool_access', category).then(() => { window.location.href = `tools.html?category=${category}`; });
}
...
function processToolAccess() {
    ...
    if (currentUserData && currentUserData.accountStatus === 'disabled') { closeModal(); showToast('Account disabled!', 'error'); return; }
    if (!currentUserData || currentUserData.credits < 5) { closeModal(); showInsufficientCredits(5); return; }
    isProcessingTool = true;
    deductCredits(5, 'tool_open', toolName).then(() => { isProcessingTool = false; closeModal(); window.open(toolLink, '_blank'); })
...
function deductCredits(amount, action, details) {
    ...
    return db.runTransaction((transaction) => {
        ...
        db.collection('transactions').add({ userId: currentUser.uid, userEmail: currentUser.email, action, details, amount: -amount, timestamp: firebase.firestore.FieldValue.serverTimestamp() }).catch(() => {});
...
function closeInsufficientModal() { document.getElementById('insufficientModal').style.display = 'none'; }
